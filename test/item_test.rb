...
class ItemTest < Minitest::Test
  ...
  def test_it_returns_an_array_of_invoice_items
    item = Invoice.new(item_repo.find_by_id(11), db)
  ...
  def test_invoice_items__it_returns_the_correct_invoice_items
    item = Invoice.new(item_repo.find_by_id(127), db)
    ...
    assert_equal [606, 607], invoice_item_ids
    assert_equal [6, 4], quantities
  end

  def test_merchant__it_can_pull_a_merchant
    item = Invoice.new(item_repo.find_by_id(9), db)

    assert_equal Merchant, item.merchant.class
  end

  def test_merchant__it_pulls_the_correct_merchant
    item = Invoice.new(item_repo.find_by_id(302), db)
